...
import { AlertTriangle, FileText, ListFilter, Mail, Settings } from "lucide-react";
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
          Email administration is restricted to Master Admin.
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="border-b border-slate-200 pb-5">
        <div className="flex items-center gap-2 text-brand-red">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-brand-charcoal">Email Administration &amp; Automation</h1>
        </div>
      ...
      <div className="overflow-x-auto border-b border-slate-200">
        <div className="flex min-w-max space-x-1 sm:space-x-4">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 border-b-2 px-3 py-2.5 text-xs font-bold transition sm:px-4 sm:text-sm ${
                  isActive ? "border-brand-red text-brand-red" : "border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-800"
                }`}
              >
